"""Draw method for GremlinMagic"""


import networkx as nx


def draw_simple_graph(graph, names_map, labels_map, node_type_attr='type',
    ...
    _nodes = list(map(str,graph.nodes()))
    # Add names/labels to nodes
    name_map = {}
    label_map = {}
    for i in range(len(_nodes)):
        node = int(_nodes[i])
        name_map[node] = names_map[i].value
        label_map[node] = labels_map[i]
    ...
    pos = nx.spring_layout(graph, weight="hw_label", iterations=100, k=k)
    if show_edge_labels:
    ...
    nx.draw_networkx_labels(graph, pos=pos, labels=name_map)
